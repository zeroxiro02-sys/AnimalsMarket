...
def _seed(db):
    ...
    sellers = [
        ('sardor',hp('demo123'),'Sardor Abdullayev','+998901234567','Samarqand',"10 yildan beri chorvachilik bilan shug'ullanaman.",4.9,23,1,'S'),
        ('aziz',hp('demo123'),'Aziz Karimov','+998912345678',"Toshkent vil.","Qo'y va echki fermasi. Barcha hayvonlar emlangan.",4.7,15,1,'A'),
        ('malika',hp('demo123'),'Malika Yusupova','+998933456789','Buxoro',"Oilaviy ferma. 20 yillik tajriba.",4.8,31,1,'M'),
        ('bobur',hp('demo123'),'Bobur Toshmatov','+998944567890',"Farg'ona","Otchilik mutaxassisi. Zotli otlar.",4.6,8,0,'B'),
    ]
    for row in sellers:
    ...
    listings = [
        (uids['sardor'],'Sut sigiri — Holstein zoti','sigir','🐄',"Sigir",12000000,'Samarqand','Urgut','4 yosh',"Urg'ochi",'Holstein',1,'350 kg',"Kuniga 25 litr sut beradi. Sog'lom, emlangan.",1,1),
        (uids['aziz'],"Qo'y (3 bosh)",'qoy','🐑',"Qo'y",4500000,"Toshkent vil.",'Zangiota','2-3 yosh','Aralash',"O'zbek zoti",3,'65 kg',"Uchta qo'y. Sog'lom, semiz, emlangan.",1,0),
        (uids['bobur'],'Zotli Arab oti','ot','🐴',"Ot",25000000,"Farg'ona","Marg'ilon",'5 yosh','Erkak','Arab zoti',1,'480 kg',"Toza zotli, mashg'ullanilgan, hujjatli.",1,1),
        (uids['aziz'],'Echki (2 bosh) — sut zoti','echki','🐐',"Echki",2800000,"Toshkent vil.",'Kibray','1-2 yosh',"Urg'ochi",'Zanen',2,'42 kg',"Sut echkilari. Har biri kuniga 3 litr sut.",1,0),
        (uids['malika'],'Broiler tovuq (20 bosh)','tovuq','🐓',"Tovuq",1200000,'Buxoro','Buxoro sh.','45 kunlik','Aralash','Broiler',20,'2.5 kg',"Tayyor savdo uchun.",1,0),
        (uids['sardor'],"Qoramol — go'sht uchun",'qoramol','🐂',"Qoramol",8500000,'Samarqand','Samarqand sh.','3 yosh','Erkak','Aralash',1,'380 kg',"380 kg. Go'sht uchun semirtirilgan.",1,0),
        (uids['malika'],"Qo'y (5 bosh) — Gissar",'qoy','🐑',"Qo'y",9000000,'Buxoro','Kogon','2-4 yosh','Erkak','Gissar',5,'80 kg',"Gissar zoti. Semiz, sog'lom.",1,1),
        (uids['bobur'],'Sut sigiri — Simmental','sigir','🐄',"Sigir",15000000,"Farg'ona","Farg'ona sh.",'5 yosh',"Urg'ochi",'Simmental',1,'420 kg',"Oyiga 700 litr sut.",1,0),
    ]
    for l in listings:
        db.execute("""INSERT INTO listings
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", l[:16]+(secrets.randbelow(300)+20,))
    db.execute("INSERT INTO reviews (from_id,to_id,rating,body) VALUES (?,?,?,?)",
               (uids['aziz'],uids['sardor'],5,"Juda ishonchli sotuvchi!"))
    db.execute("INSERT INTO reviews (from_id,to_id,rating,body) VALUES (?,?,?,?)",
               (uids['malika'],uids['sardor'],5,"Tez javob berdi."))
...
def _ctx():
    ...
    return dict(me=u, unread=unread, fav_count=favs, animal_types=ANIMAL_TYPES, regions=REGIONS, now=datetime.now())
...
@app.route('/catalog')
def catalog():
    db=get_db(); q=request.args.get('q','').strip(); atype=request.args.get('type','')
    reg=request.args.get('region',''); minp=request.args.get('min',''); maxp=request.args.get('max','')
    sort=request.args.get('sort','new'); page=max(1,int(request.args.get('page','1') or 1)); per=12
    sql="""SELECT l.*,u.full_name,u.rating,u.is_verified,u.username
           FROM listings l JOIN users u ON l.user_id=u.id WHERE l.is_active=1 AND l.is_sold=0"""
    prm=[]
    ...
    ord_map={'new':'l.created_at DESC','price_asc':'l.price ASC','price_desc':'l.price DESC','popular':'l.views DESC'}
    sql+=f" ORDER BY l.is_premium DESC,{ord_map.get(sort,'l.created_at DESC')}"
    all_items=db.execute(sql,prm).fetchall(); total=len(all_items)
    pages=max(1,(total+per-1)//per); items=all_items[(page-1)*per:page*per]
    return render_template('main/catalog.html',items=items,total=total,pages=pages,page=page,
                           q=q,atype=atype,reg=reg,minp=minp,maxp=maxp,sort=sort)

@app.route('/delivery')
def delivery():
    db=get_db(); region=request.args.get('region',''); district=request.args.get('district','')
    q=request.args.get('q','').strip(); atype=request.args.get('type','')
    page=max(1,int(request.args.get('page','1') or 1)); per=12
    sql="""SELECT l.*,u.full_name,u.rating,u.is_verified,u.username
           FROM listings l JOIN users u ON l.user_id=u.id WHERE l.is_active=1 AND l.is_sold=0"""
    prm=[]
    ...
    return render_template('main/delivery.html',items=items,total=total,pages=pages,page=page,
                           q=q,atype=atype,region=region,district=district,regions=REGIONS,
                           districts=districts,all_districts=UZBEKISTAN_DISTRICTS,animal_types=ANIMAL_TYPES)
...
def listing(lid):
    ...
    seller_other=db.execute("""SELECT * FROM listings
        WHERE user_id=? AND is_active=1 AND id!=? AND is_sold=0 LIMIT 3""",(item['seller_id'],lid)).fetchall()
    is_fav=False
    if 'uid' in session:
        is_fav=bool(db.execute("SELECT 1 FROM favorites WHERE user_id=? AND listing_id=?",(session['uid'],lid)).fetchone())
    ...
    return render_template('main/listing.html',item=item,related=related,seller_other=seller_other,
                           is_fav=is_fav,reviews=rev,delivery_persons=DELIVERY_PERSONS,vets=VETS)

@app.route('/delivery-review', methods=['POST'])
def delivery_review():
    if not session.get('uid'): flash('Iltimos tizimga kiring','error'); return redirect('/login')
...
def new_listing():
    if request.method=='POST':
        ...
        aslug=request.form.get('animal_slug',''); price=int(request.form.get('price',0) or 0)
...
def edit_listing(lid):
    db=get_db(); item=db.execute("SELECT * FROM listings WHERE id=? AND user_id=?",(lid,session['uid'])).fetchone()
    if not item: abort(403)
    if request.method=='POST':
        aslug=request.form.get('animal_slug',item['animal_slug'])
        em={s:e for s,e,_ in ANIMAL_TYPES}.get(aslug,'🐾'); an={s:n for s,_,n in ANIMAL_TYPES}.get(aslug,'')
        db.execute("""UPDATE listings SET title=?,animal_slug=?,animal_emoji=?,animal_name=?,
                      ...
                      WHERE id=?""",(request.form.get('title',item['title']),aslug,em,an,
            int(request.form.get('price',item['price']) or 0),request.form.get('region',item['region']),
            request.form.get('district',''),request.form.get('age',''),request.form.get('gender',''),
            request.form.get('breed',''),int(request.form.get('count',1) or 1),
            request.form.get('weight',''),request.form.get('description','').strip(),lid,))
        db.commit(); flash("E'lon yangilandi ✅",'success'); return redirect(url_for('listing',lid=lid))
...
def delete_listing(lid):
    db=get_db(); item=db.execute("SELECT * FROM listings WHERE id=?",(lid,)).fetchone()
    if not item: abort(404)
    u=_me()
    if item['user_id']!=session['uid'] and not u['is_admin']: abort(403)
    db.execute("DELETE FROM favorites WHERE listing_id=?",(lid,))
    db.execute("DELETE FROM listings WHERE id=?",(lid,)); db.commit()
...
def mark_sold(lid):
    ...
    db.commit(); flash("E'lon 'Sotildi' deb belgilandi ✅",'success'); return redirect(url_for('my_listings'))
...
def bron_listing(lid):
    db=get_db(); item=db.execute("SELECT * FROM listings WHERE id=?",(lid,)).fetchone()
    if not item: abort(404)
    if item['is_sold']: flash("Bu e'lon allaqachon sotilgan",'error'); return redirect(url_for('listing',lid=lid))
    if item['user_id']==session['uid']: flash("O'z e'loningizni bron qila olmaysiz",'error'); return redirect(url_for('listing',lid=lid))
    if item['is_reserved']:
        if item['reserved_by']==session['uid']:
            ...
            db.commit(); flash("Bron bekor qilindi",'info')
        else: flash("Bu e'lon allaqachon bronlangan",'error')
        return redirect(url_for('listing',lid=lid))
    db.execute("UPDATE listings SET is_reserved=1,reserved_by=?,reserved_at=datetime('now') WHERE id=?",(session['uid'],lid))
    db.commit(); flash("E'lon muvaffaqiyatli bronlandi!",'success'); return redirect(url_for('listing',lid=lid))
...
def cancel_bron(lid):
    ...
    if not item: abort(404)
    me=_me()
    if item['reserved_by']!=session['uid'] and not me['is_admin']: abort(403)
    db.execute("UPDATE listings SET is_reserved=0,reserved_by=NULL,reserved_at=NULL WHERE id=?",(lid,))
    db.commit(); flash("Bron bekor qilindi",'info'); return redirect(url_for('listing',lid=lid))
...
def confirm_click_payment(lid):
    ...
    db.commit(); flash("💳 Click orqali to'lov muvaffaqiyatli amalga oshirildi!",'success')
    return redirect(url_for('listing',lid=lid))
...
def toggle_fav(lid):
    db=get_db()
    ex=db.execute("SELECT 1 FROM favorites WHERE user_id=? AND listing_id=?",(session['uid'],lid)).fetchone()
    if ex: db.execute("DELETE FROM favorites WHERE user_id=? AND listing_id=?",(session['uid'],lid)); action='removed'
    else: db.execute("INSERT OR IGNORE INTO favorites(user_id,listing_id) VALUES(?,?)",(session['uid'],lid)); action='added'
    db.commit()
    cnt=db.execute("SELECT COUNT(*) FROM favorites WHERE user_id=?",(session['uid'],)).fetchone()[0]
    if request.headers.get('X-Requested-With')=='XMLHttpRequest': return jsonify({'action':action,'count':cnt})
...
def messages():
    db=get_db()
    convs=db.execute("""SELECT u.id,u.full_name,u.username,u.avatar_letter,
        ...
        FROM messages m JOIN users u ON (CASE WHEN m.from_id=? THEN m.to_id ELSE m.from_id END)=u.id
        WHERE m.from_id=? OR m.to_id=? GROUP BY u.id ORDER BY last_time DESC""",
        (session['uid'],)*6).fetchall()
    return render_template('main/messages.html',convs=convs)

@app.route('/messages/<int:uid>', methods=['GET','POST'])
@login_required
def chat(uid):
    db=get_db(); other=db.execute("SELECT * FROM users WHERE id=?",(uid,)).fetchone()
    if not other: abort(404)
    if request.method=='POST':
        body=request.form.get('body','').strip(); lid=request.form.get('listing_id') or None
        if body: db.execute("INSERT INTO messages(from_id,to_id,listing_id,body) VALUES(?,?,?,?)",(session['uid'],uid,lid,body)); db.commit()
        return redirect(url_for('chat',uid=uid))
    db.execute("UPDATE messages SET is_read=1 WHERE from_id=? AND to_id=?",(uid,session['uid'])); db.commit()
    msgs=db.execute("""SELECT m.*,u.full_name,u.avatar_letter FROM messages m JOIN users u ON m.from_id=u.id
        WHERE (m.from_id=? AND m.to_id=?) OR (m.from_id=? AND m.to_id=?) ORDER BY m.created_at""",
        (session['uid'],uid,uid,session['uid'])).fetchall()
...
def profile():
    ...
    if request.method=='POST':
        ...
        db.execute("UPDATE users SET full_name=?,phone=?,region=?,email=?,bio=?,avatar_letter=? WHERE id=?",
                   (fn,request.form.get('phone','').strip(),request.form.get('region',''),
                    request.form.get('email','').strip(),request.form.get('bio','').strip(),
                    fn[0].upper() if fn else '?',session['uid']))
        op=request.form.get('old_password',''); np=request.form.get('new_password','')
        if op and np:
            u=db.execute("SELECT * FROM users WHERE id=? AND password=?",(session['uid'],hp(op))).fetchone()
            if u: db.execute("UPDATE users SET password=? WHERE id=?",(hp(np),session['uid'])); flash("Parol o'zgartirildi ✅",'success')
            else: flash("Eski parol noto'g'ri",'error')
        db.commit(); flash("Profil yangilandi ✅",'success'); return redirect(url_for('profile'))
...
@login_required
def my_listings():
    items=get_db().execute("SELECT * FROM listings WHERE user_id=? ORDER BY created_at DESC",(session['uid'],)).fetchall()
...
def user_page(username):
    ...
    return render_template('main/user.html',user=user,items=items,reviews=revs,avg=round(avg or 5.0,1))

@app.route('/user/<int:uid>/review', methods=['POST'])
@login_required
def add_review(uid):
    db=get_db(); rating=int(request.form.get('rating',5)); body=request.form.get('body','').strip()
    if uid==session['uid']: flash("O'zingizga baho bera olmaysiz",'error'); return redirect(request.referrer or url_for('home'))
    db.execute("INSERT INTO reviews(from_id,to_id,rating,body) VALUES(?,?,?,?)",(session['uid'],uid,rating,body))
    ...
    db.commit(); flash("Sharh qo'shildi ✅",'success'); return redirect(request.referrer or url_for('home'))

@app.route('/login', methods=['GET','POST'])
def login():
    if 'uid' in session: return redirect(url_for('home'))
    if request.method=='POST':
        uname=request.form.get('username','').strip(); pwd=request.form.get('password','').strip()
        user=get_db().execute("SELECT * FROM users WHERE username=? AND password=?",(uname,hp(pwd))).fetchone()
        if user:
...
@app.route('/logout')
def logout():
    session.pop('uid',None); flash("Tizimdan chiqildi 👋",'info'); return redirect(url_for('home'))

@app.route('/register', methods=['GET','POST'])
def register():
    if 'uid' in session: return redirect(url_for('home'))
    if request.method=='POST':
        uname=request.form.get('username','').strip(); fn=request.form.get('full_name','').strip()
        phone=request.form.get('phone','').strip(); reg=request.form.get('region','')
        pwd=request.form.get('password','').strip(); pwd2=request.form.get('password2','').strip()
        if not all([uname,fn,phone,reg,pwd]): flash("Barcha maydonlarni to'ldiring",'error')
        elif pwd!=pwd2: flash("Parollar mos emas",'error')
        elif len(pwd)<6: flash("Parol kamida 6 ta belgi",'error')
        elif get_db().execute("SELECT 1 FROM users WHERE username=?",(uname,)).fetchone(): flash("Bu login band, boshqasini tanlang",'error')
        else:
            db=get_db(); role=request.form.get('role','buyer')
            ...
            session['uid']=uid; flash(f"Xush kelibsiz, {fn}! Ro'yxatdan o'tdingiz 🎉",'success')
...
def api_search():
    ...
    r=get_db().execute("""SELECT id,title,animal_emoji,price,region FROM listings
        WHERE (title LIKE ? OR breed LIKE ?) AND is_active=1 AND is_sold=0 LIMIT 6""",(f'%{q}%',f'%{q}%')).fetchall()
    return jsonify([dict(id=x['id'],title=x['title'],emoji=x['animal_emoji'],price=x['price'],region=x['region']) for x in r])
...
def api_ai_animal_info():
    ...
    animal_name=data.get('animal_name',''); breed=data.get('breed',''); age=data.get('age','')
    region=data.get('region',''); image_b64=data.get('image_b64',''); description=data.get('description','')
    ...
    system_msg=('Sen O\'zbekiston hayvonlar bozori uchun mutaxassis AI yordamchisisan. '
        'JSON formatda javob ber (faqat JSON):\n{"animal_info":"...","breed_info":"...","price_min":500000,'
        '"price_max":2000000,"price_avg":1200000,"price_explanation":"...","care_tips":["..."],'
        '"health_tips":["..."],"buy_tips":["..."],"confidence":85}')
    user_content=[]
    if image_b64:
        if ',' in image_b64: image_b64=image_b64.split(',',1)[1]
        user_content.append({"type":"image","source":{"type":"base64","media_type":"image/jpeg","data":image_b64}})
    user_content.append({"type":"text","text":"\n".join(prompt_parts)})
    import urllib.request as ur
    payload=json.dumps({"model":"claude-sonnet-4-20250514","max_tokens":1000,"system":system_msg,
    ...
    try:
        with ur.urlopen(req,timeout=30) as resp: result=json.loads(resp.read())
...
def gps_map():
    db=get_db()
    listings=db.execute("""SELECT l.id,l.title,l.animal_emoji,l.price,l.region,l.district,
               l.latitude,l.longitude,u.username,u.full_name
        FROM listings l JOIN users u ON l.user_id=u.id
        WHERE l.is_active=1 AND l.is_sold=0 ORDER BY l.created_at DESC""").fetchall()
    data=[dict(id=x['id'],title=x['title'],emoji=x['animal_emoji'],price=x['price'],
               region=x['region'],district=x['district'],lat=x['latitude'],lng=x['longitude'],
...
def set_listing_location(lid):
    data=request.get_json(force=True); lat=data.get('lat'); lng=data.get('lng')
    db=get_db(); listing=db.execute("SELECT * FROM listings WHERE id=?",(lid,)).fetchone()
    if not listing: return jsonify({"ok":False,"error":"Not found"}),404
    me=_me()
    if listing['user_id']!=me['id'] and not me['is_admin']: return jsonify({"ok":False,"error":"Forbidden"}),403
    db.execute("UPDATE listings SET latitude=?,longitude=? WHERE id=?",(lat,lng,lid)); db.commit()
...
@app.route('/contact')
def contact(): return render_template('main/contact.html')
...
def admin_index():
    db=get_db()
    stats=dict(users=db.execute("SELECT COUNT(*) FROM users WHERE is_admin=0").fetchone()[0],
    ...
    return render_template('admin/dashboard.html',stats=stats,recent_users=recent_users,recent_listings=recent_listings)
...
def admin_verify(uid):
    ...
    if u: db.execute("UPDATE users SET is_verified=? WHERE id=?",(0 if u['is_verified'] else 1,uid)); db.commit()
...
def admin_delete_user(uid):
    if uid==session['uid']: flash("O'zingizni o'chira olmaysiz",'error'); return redirect(url_for('admin_users'))
    db=get_db()
    for q in ["DELETE FROM listings WHERE user_id=?","DELETE FROM messages WHERE from_id=? OR to_id=?",
              "DELETE FROM favorites WHERE user_id=?","DELETE FROM reviews WHERE from_id=? OR to_id=?",
              "DELETE FROM users WHERE id=?"]:
        db.execute(q,(uid,uid) if 'OR' in q else (uid,))
    db.commit(); flash("Foydalanuvchi o'chirildi",'info'); return redirect(url_for('admin_users'))
...
def admin_toggle_listing(lid):
    ...
    if item:
        db.execute("UPDATE listings SET is_active=?,is_premium=? WHERE id=?",
                   (0 if item['is_active'] else 1,1 if request.form.get('premium') else item['is_premium'],lid))
        db.commit()
...
def admin_del_listing(lid):
    ...
    db.commit(); flash("E'lon o'chirildi",'info'); return redirect(url_for('admin_listings'))
