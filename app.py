...
def init_db():
    ...
    if 'google_id' not in ucols:
        db.execute("ALTER TABLE users ADD COLUMN google_id TEXT DEFAULT NULL")
...
def new_listing():
    if request.method == 'POST':
        ...
        if not all([title,aslug,price,region]):
            ...
            return render_template('main/new_listing.html')
        ...
        db.execute("""INSERT INTO listings
            (user_id,title,animal_slug,animal_emoji,animal_name,price,region,
             district,age,gender,breed,count,weight,description,latitude,longitude)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", (
...
def google_callback():
    try:
        ...
        flash("Google orqali kirishda xatolik yuz berdi", "error")
    ...
    if not user:
        ...
        base_username = re.sub(r'[^a-z0-9]', '', full_name.lower().replace(' ', '_')) or 'user'
        ...
        db.execute(
            "INSERT INTO users (username, password, full_name, email, avatar_letter, google_id, is_verified) "
            "VALUES (?, ?, ?, ?, ?, ?, 1)",
            (username, '', full_name, email, avatar, google_id)
        )
        db.commit()
        user = db.execute("SELECT * FROM users WHERE google_id=?", (google_id,)).fetchone()
